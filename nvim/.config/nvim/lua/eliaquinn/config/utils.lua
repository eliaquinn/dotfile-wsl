...
function M.show_last_commit_diff()
  ...
  local git_root = vim.fn.system("git -C " ..
    vim.fn.shellescape(vim.fn.expand("%:p:h")) .. " rev-parse --show-toplevel 2>/dev/null"):gsub("\n", "")
  ...
  local hash = vim.fn.system(
    string.format("git -C %s log -1 --pretty=format:%%H -- %s 2>/dev/null",
  ...
  local output = vim.fn.system(
    string.format("git -C %s show %s -- %s 2>/dev/null",
      vim.fn.shellescape(git_root),
      hash,
      vim.fn.shellescape(rel_path))
  )
